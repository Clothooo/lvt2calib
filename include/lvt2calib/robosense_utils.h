#ifndef robosense_utils_H
#define robosense_utils_H
...
enum FIX_LASER_TYPE { RS_16 = 0, RS_32, RS_48, RS_80, RS_128 };


static const int RINGS_COUNT_RS16 = 16; // TODO AS FUNCTION PARAM
static const int RINGS_COUNT_RS32 = 32;
static const int RINGS_COUNT_RS48 = 48;
static const int RINGS_COUNT_RS80 = 80;
static const int RINGS_COUNT_RS128 = 128;
...
FIX_LASER_TYPE laser_type = RS_16;
...
namespace Robosense {
  ...
  // void addRings(pcl::PointCloud<Robosense::Point> & pc)
  // {
  ...
  void addRange(pcl::PointCloud<Robosense::Point> & pc){
  ...
  vector<vector<Point*> > getRings(pcl::PointCloud<Robosense::Point> & pc, FIX_LASER_TYPE type_ = RS_16)
  {
  ...
  void resetIntensity(pcl::PointCloud<Robosense::Point> & pc)
  {
    vector<vector<Robosense::Point*> > rings = Robosense::getRings(pc, laser_type);
    cout << "rings.size = " << rings.size() << endl;
    for (vector<vector<Robosense::Point *>>::iterator ring = rings.begin(); ring < rings.end(); ++ring)
    {
      if (ring->empty()) continue;

      (*ring->begin())->intensity = 0;
      (*(ring->end() - 1))->intensity = 0;
      for (vector<Robosense::Point *>::iterator pt = ring->begin() + 1; pt < ring->end() - 1; pt++)
      {
        Robosense::Point *prev = *(pt - 1);
        Robosense::Point *succ = *(pt + 1);
  ...
  void normalizeIntensity(pcl::PointCloud<Point> & pc, float minv = 0, float maxv = 255)
  {
    ...
    for (pcl::PointCloud<Point>::iterator pt = pc.points.begin(); pt < pc.points.end(); pt++)
    {
      max_found = max(max_found, (float)pt->intensity);
      min_found = min(min_found, (float)pt->intensity);
    }
  ...
  void copyPointCloud(const pcl::PointCloud<Robosense::Point> &cloud_in, pcl::PointCloud<pcl::PointXYZI> &cloud_out)
  {
      ...
      auto it_out = cloud_out.points.begin();
      auto it_in = cloud_in.points.begin();
      for (; it_out < cloud_out.points.end() && it_in < cloud_in.points.end(); ++it_out, ++it_in)
      {
        it_out->x = it_in->x;
        it_out->y = it_in->y;
        it_out->z = it_in->z;
        it_out->intensity = (float)it_in->intensity;
      }
      return;
  }
}
...
void findLaserType(int laser_ring_num)
{
  ...
  if(it != rings_count_v.end())
  {
    ...
    case 0:
      laser_type = RS_16;
      cout << "laser_type: RS_16" << endl;
      break;
    case 1:
      laser_type = RS_32;
      cout << "laser_type: RS_32" << endl;
      break;
    case 2:
      laser_type = RS_48;
      cout << "laser_type: RS_48" << endl;
      break;
    case 3:
      laser_type = RS_80;
      cout << "laser_type: RS_80" << endl;
      break;
    case 4:
      laser_type = RS_128;
      cout << "laser_type: RS_128" << endl;
      break;
