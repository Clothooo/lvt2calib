...
#include <lvt2calib/robosense_utils.h>
...
typedef Robosense::Point PointType;
...
void callback(const PointCloud2::ConstPtr& laser_cloud)
{
    ...
    Robosense::addRange(*cloud_in);
    // Robosense::normalizeIntensity(*cloud_in, 0, 255);
    pcl::copyPointCloud(*cloud_in, *cloud_in_copy);
    // Robosense::copyPointCloud(*cloud_in, *cloud_in_copy);
    publishPC<PointType>(cloud_in_pub, cloud_header, cloud_in);

    Robosense::resetIntensity(*cloud_in);
